...
export const BackgroundGradientAnimation1 = ({
  ...
  useEffect(() => {
    ...
    const gradientStartLocation = gl.getUniformLocation(program, "u_gradientStart");
    ...
    const parseRGB = (rgbString: string) => {
      const values = rgbString.split(",").map(val => parseInt(val.trim()) / 255);
      return values;
    };
    ...
    const parseGradientColor = (colorString: string) => {
      if (colorString.startsWith("rgb(")) {
        const values = colorString.replace("rgb(", "").replace(")", "").split(",");
